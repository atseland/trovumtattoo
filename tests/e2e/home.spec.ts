...
test('booking info page points users to the booking form before price details', async ({ page }) => {
  ...
  const bookingRequestLinks = page.getByRole('link', { name: 'Send bookingforespørsel' })
  await expect(bookingRequestLinks).toHaveCount(2)
  await expect(bookingRequestLinks.first()).toHaveAttribute('href', '/book')
  await expect(bookingRequestLinks.nth(1)).toHaveAttribute('href', '/book')
